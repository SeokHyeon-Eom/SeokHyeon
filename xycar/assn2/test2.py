...
while True:
    ...
    for m in range(left_start + area_width * 2, right_start + area_width * 2):
        area = bin_1[row_begin:row_end, m: m + area_width]
        if cv2.countNonZero(area) > 10:
            mid_start = m
            right_square2 = cv2.rectangle(edge,
                                          (m, row_begin),
                                          (m +  area_width, row_end),
                                          (0, 255, 255), 2)
            break
    # test end
    ...
    for m in range(mid_start, rmid - area_width):
        area = bin[row_begin:row_end, m: m + area_width]
        if cv2.countNonZero(area) > pixel_cnt_threshold:
            mid = m
            break
    ...
    if mid != -1:
        mid_square = cv2.rectangle(frame,
                                     (mid, roi_vertical_pos + row_begin),
                                     (mid + area_width, roi_vertical_pos + row_end),
                                     (0, 255, 255), 2)

        msquare = cv2.rectangle(view,
                                (mid, row_begin),
                                (mid + area_width, row_end),
                                (0, 255, 255), 2)
